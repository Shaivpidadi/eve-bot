...
const SYSTEM: Record<CaptureMode, string> = {
  ...
  conversation: [
    ...
    "- set: a field's value, when the person stated or clearly implied it (their name, company, location, timezone, spelling, date or time format, writing rules such as 'no em dashes', who approves what, where tickets live, and so on). The writing field collects every always-or-never rule about prose, semicolon-separated; setting it replaces it, so carry the existing rules forward and add the new one. Setting a field that already has a value replaces it. Prefer set over add whenever a field fits, and set a field that is (unset) even when a note already says the same thing: fields are where these facts belong. Use the field keys exactly as listed.",
